...
def convert_to_mp4(input_file_path, output_file_path, quality='medium', resolution='original'):
    ...
    ffmpeg_path = r'/usr/bin/ffmpeg'  # Укажите полный путь до ffmpeg.exe
    command = [
...
def get_video_duration(file_path):
    """
    Получает длительность видео в секундах с помощью FFmpeg.
    """
    command = [r'/usr/bin/ffmpeg', '-i', file_path]
...
def upload_video(request):
    ...
    if request.method == 'POST':
        ...
        if not target_language:
            messages.error(request, 'No target language selected')
            return redirect(request.path)
        ...
        try:
            ...
            if profile.tokens < tokens_needed:
                messages.error(request, 'There are not enough tokens to process this video')
                return redirect(request.path)
            ...
            logging.error(f"Ошибка при определении длительности видео: {e}")
            messages.error(request, f'Ошибка при определении длительности видео: {e}')
            return redirect(request.path)

        # Здесь продолжается остальной код обработки видео...
        ...
        try:
            ...
            # Получаем имя выходного видеофайла сразу после его создания
            output_video_filename = os.path.basename(output_video_path)

            # Создание миниатюры после обработки видео
            thumbnail_path = create_thumbnail(output_video_path, dubbing_id, f"{output_video_filename}.png")


            download_link = f'/download/{dubbing_id}/{output_video_filename}'
            current_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            filename_with_date = f"[{current_date}] {output_video_filename}"

            video_link = VideoLink(
                user=request.user,
                link=download_link,
                filename=filename_with_date,
                file_path=output_video_path,  # Путь к видео
                thumbnail_path=thumbnail_path  # Путь к миниатюре
            )
            video_link.save()
            submitted = True
            ...
            messages.error(request, f'Ошибка при обработке видео: {e}')
            return redirect(request.path)

        os.remove(file_path)

        # Обратите внимание, как мы теперь передаем профиль в контекст
        return render(request, 'main/upload.html', {
